...
Copyright: 2018, Jan ZĂ¶rner
Licence: GNU GPLv3
"""

#______________Importowanie potrzebnych bibliotek______________#
import time
import platform
import warnings
...
class PyCrown:
    ...
    __author__ = "Dr. Jan ZĂ¶rner"
    __copyright__ = "Copyright 2018, Jan ZĂ¶rner"
    __credits__ = ["Jan ZĂ¶rner", "John Dymond", "James Shepherd", "Ben Jolly"]
    __license__ = "GNU GPLv3"
    __version__ = "0.1"
    __maintainer__ = "Jan ZĂ¶rner"
    ...
    def crowns_to_polys_smooth(self, store_las=True, thin_perc=None,
        ...
        for tidx in range(len(self.trees)):
            ...
            if len(points.z) > 1 and not np.allclose(points.z,
                                                     points.iloc[0].z):
                points = points[points['z'].to_numpy() >= threshold_otsu(points['z'].to_numpy())]
                if first_return:
